...
if ($sdateDe>$emiday) {
	...
	echo "<hr/>";
	die();*/
	$sdateFirstEmi =$sdateFirstEmiM."/".$emiday."/".$sdateFirstEmiY;
	
	$sdateFirstEmistrtotime =strtotime($sdateFirstEmi);
	$edatestrtotime =strtotime($edate);
	$extraDayFlag = ($edatestrtotime-$sdateFirstEmistrtotime)/86400;
	if ($extraDayFlag<0){
		$dayExtra =0;
	}
	//die(); 
		...
		/*echo $emiday;
		die();*/
		$dayExtra =($emiday-$sdateDe);
		$edatestrtotime =strtotime($edate);
		$sdateFirstEmi =$sdateMe."/".$emiday."/".$sdateYe;
		$sdateFirstEmistrtotime =strtotime($sdateFirstEmi);
		$extraDayFlag = ($edatestrtotime-$sdateFirstEmistrtotime)/86400;
		if ($extraDayFlag<0){
			$dayExtra =0;
...
function calulateTotalPayment($sdate,$edate,$emiday,$famount,$frate) {
	...
	$totalPayment['extraMoney'] =round($extraMoney);
	/*echo "<pre>";
...
echo $emir;
echo "<hr/>";*/
$unadR =$totalReceiveFinance-$totalPayment['totalPayment'];
if ($unadR>0) {
	$unadjustmentAmount+=$unadR;
}else {
	$unadjustmentAmount+=abs($totalReceiveFinance%$finance['monthlyEmi']);
}
$finance['financeDueMonth']= $emimonth-$emir;
/*echo "<hr/>";
echo $finance['monthlyEmi'];
echo "<hr/>";
echo $finance['financeDueMonth'];
echo "<hr/>";
echo $totalPayment['extraMoney'];*/
...
/*echo "<hr/>";
echo $finance['dueEmiAmt'];
echo "<hr/>";
echo $finance['financeDueMonth'];
die();*/

if ($finance['financeDueMonth']<0) {
	$finance['financeDueMonth'] =0;
}
if ($finance['dueEmiAmt']<0) {
	$finance['dueEmiAmt'] =0;
}
/*echo "<pre>";
print_r($finance);
...
//echo "<pre>";print_r($_REQUEST);die();
	...
	$unadR =$recrent['totalReceiveRent']-$recrent['totalRent'];
	if ($unadR>0) {
		$unadjustmentAmount+=$unadR;
	}else {

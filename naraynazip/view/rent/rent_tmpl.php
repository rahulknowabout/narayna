...
if(isset($_GET['vid']) && $_GET['vid']!="") {
	...
	$counter= $vid1+1;
	$vid1 = ($_GET['vid']-1)*10;
	$rent_info = $ketObj->runquery( "SELECT", "np.title,nr.rent_id,nr.rent_name,rent_address,rent_mobile,nu.uname", "nar_rent nr INNER JOIN nar_property np ON np.pid = nr.rent_property_id INNER JOIN nar_user nu ON nu.uid = nr.rent_deal_person_id", array(), $where." limit ".$vid1.",10"  );
	//$rowe = runquery("SELECT","*","artical","","".$where."limit ".$vid1.",10");
    //$count= $vid1+1;
	}
else {
	$rent_info = $ketObj->runquery( "SELECT", "np.title,nr.rent_id,nr.rent_name,nr.rent_address,nr.rent_mobile,nu.uname", "nar_rent nr INNER JOIN nar_property np ON np.pid = nr.rent_property_id INNER JOIN nar_user nu ON nu.uid = nr.rent_deal_person_id", array(), $where." limit 0,10"  );
}
$qstring="";
if( isset( $_REQUEST['searchbyorder'] ) && $_REQUEST['searchbyorder'] != "" ){	

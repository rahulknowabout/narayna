...
if( isset($searchbyorder ) && $searchbyorder!="" ) {
	...
	$wherefinp =" where str_to_date(fin_rec_date,'%m/%d/%Y') <= str_to_date('".$pred."','%m/%d/%Y')";
	$wherefin = " where fin_rec_date ='".$searchbyorder."'";
	$whererent = " where rec_date ='".$searchbyorder."'";
	$whererentp = " where str_to_date(rec_date,'%m/%d/%Y') <=str_to_date('".$pred."','%m/%d/%Y')";
	$whereexp= " where exp_date ='".date("d/m/Y",strtotime("".$searchbyorder.""))."'";
	$whereexpop= " where str_to_date(exp_date,'%d/%m/%Y') <= str_to_date('".date("d/m/Y",strtotime("".$pred.""))."','%d/%m/%Y')";
...
$opbalanceE =$ketObj->runquery( "SELECT","sum(exp_amount) as amt", "nar_expansion", array(),"".$whereexpop."","")['0']['amt'];
...
$opbalanceE =$opbalanceE==""?0:$opbalanceE;
$opbalnce =$opbalancef+$opbalancer-$opbalanceE;
$recfinance =$recfinance==""?0:$recfinance;
$recrent =$recrent==""?0:$recrent;

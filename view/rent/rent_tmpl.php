...
$(document).ready(function(){
 $.fn.myFunction = function(){ 
	  ...
	  counter=document.getElementById('continue').value;
      ...
      type: 'post',
      url: 'index.php',
     data: "aj=y&chk=pagination&onlink=Rlist&getresult="+val+"&where="+where+"&tilldate="+tilldate+"&counter="+counter,
     beforeSend: function(){
			$('#loader-icon').show();
			},
	complete: function(){
	...
	//alert(data);
	document.getElementById('continue').value =Number(counter)+15;
		//alert(document.getElementById('continue').value);
			if (data === "no") {
				$('#loader-icon').hide();
			}else {
	...
	if($(document).height() <= $(window).scrollTop() + $(window).height()){
		if (tilldate !== "") {
	  		setTimeout($.fn.myFunction, 500)	//$.fn.myFunction();
		}else {
			$.fn.myFunction();
		}	
		//loadmore();
...
if (isset($_REQUEST['tilldate'])) {
	$wherefin = str_replace("0","",date("d",strtotime(str_replace("/","-","".$_REQUEST['tilldate'].""))));
	$where .= " where nr.rent_day <=".$wherefin."";
}else {
...
if(isset($_GET['vid']) && $_GET['vid']!="") {
	...
	$rent_info = $ketObj->runquery( "SELECT", "np.title,np.pid,nr.rent_id,nr.rent_name,nr.rent_address,nr.rent_mobile,nu.uname,nr.rent_date,nr.rent_amount", "nar_rent nr INNER JOIN nar_property np ON np.pid = nr.rent_property_id INNER JOIN nar_user nu ON nu.uid = nr.rent_deal_person_id", array(), $where." limit 0,15"  );
}
$qstring="";
if( isset( $_REQUEST['searchbyorder'] ) && $_REQUEST['searchbyorder'] != "" ){	
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title">
									...
									?>
									 <li><a href="index.php?view=rent&file=duerent" class="text-primary">Due Rent List</a></li>
                                    </ul>
                                ...
                                <div class="x_content">
									...
									?>
										...
										<input type="hidden" id="tilldate" name="tilldate" value="<?php if (isset($_REQUEST['tilldate']) && $_REQUEST['tilldate'] != "" ){ echo $_REQUEST['tilldate'];}else{ echo "";} ?>">
										<input type="hidden" id="continue" name="continue" value="<?php if (isset($counter) && $counter != "" ){ echo $counter;}else{ echo "1";} ?>">
									</div>

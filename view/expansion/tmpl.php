...
$(document).ready(function(){
 $.fn.myFunction = function(){ 
      ...
      type: 'post',
      url: 'index.php',
     data: "aj=y&chk=pagination&onlink=expansion&getresult="+val+"&where="+where,
     beforeSend: function(){
			$('#loader-icon').show();
			},
	complete: function(){
...
print_r($_REQUEST);
die();*/
if (isset($_REQUEST['cstatus']) && $_REQUEST['cstatus']=="checked") {
$where = " where ne.hsta=1 AND ne.hstb=1 ";
}else{
$where = " where (ne.hsta<>1 OR ne.hstb<>1) ";
}
if( isset($_REQUEST['searchbyorder'] ) && $_REQUEST['searchbyorder']!="" ) {
	$where = "  AND (nu.uname like '%".$_REQUEST['searchbyorder']."%' OR ne.exp_date = '".$_REQUEST['searchbyorder']."')";
...
if(isset($_GET['vid']) && $_GET['vid']!="") {
	...
	$all_expansion = $ketObj->runquery( "SELECT", "ne.exp_id,ne.exp_date,ne.exp_amount,ne.otherp,ne.exp_desc,ne.hsta,ne.hstb,nu.uname", "nar_expansion ne LEFT OUTER JOIN nar_user nu ON nu.uid = ne.exp_user_id", array(), $where." limit 0,15"  );
}
$qstring="";
if( isset( $_REQUEST['searchbyorder'] ) && $_REQUEST['searchbyorder'] != "" ){	
                ...
                <div class="">
                    ...
                    <div class="page-title">
                        ...
                        <div class="title_right">
                           <form name="searchorder" id = "searchorder" action="index.php" method="post" role ="search" class="navbar-form"> <div class="row">
                            ...
                            <button class="btn btn-default" type="submit"><span class="glyphicon glyphicon-search"></span></button>
								...
								<div class="input-group">
                                    <select name="cstatus" id ="cstatus" class="form-control">
										<option value="unchecked" <?php  echo isset($_REQUEST['cstatus']) > 0 ? $_REQUEST['cstatus']== "unchecked" ? "selected='selected'":"":"";  ?>>Unchecked</option>
										<option value="checked" <?php  echo isset($_REQUEST['cstatus']) > 0 ? $_REQUEST['cstatus']== "checked" ? "selected='selected'":"":"";  ?>>Checked</option>
									</select>   
											...
											<tbody id="faq-result">
												...
												?>                     
															<tr>
																...
																<td><?php echo $row['uname']!=""?$row['uname']:$row['otherp']; ?></td> <td> <form name="checkboxboot_<?php echo $row['exp_id'];?>" id = "checkboxboot_<?php echo $row['exp_id'];?>" action="index.php" method="post"  class="form-horizontal form-label-left">

...
<?php if (isset($_REQUEST['cstatus']) && $_REQUEST['cstatus']=="checked") {
$wherefin = " where nfr.hsta=1 AND nfr.hstb=1 ";
}else{
$wherefin = " where (nfr.hstb<>1 OR nfr.hstb<>1) ";
}
if (isset($_REQUEST['cstatus']) && $_REQUEST['cstatus']=="checked") {
$whererent = " where nrc.hsta=1 AND nrc.hstb=1 ";
}else{
$whererent = " where (nrc.hstb<>1 OR nrc.hstb<>1) ";
}
$recfinance =array();
$recrent=array();
$totalMoney =0;
...
$wherefin .= " AND (nfr.fin_rec_date ='".date("m/d/Y",strtotime(str_replace("/","-","".$searchorder."")))."')";
$whererent .= " AND nrc.rec_date ='".date("m/d/Y",strtotime(str_replace("/","-","".$searchorder."")))."'";

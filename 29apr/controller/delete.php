...
}elseif (isset( $_REQUEST['chk'] ) && $_REQUEST['chk'] == 'finrecdelete') {
  if (isset( $_REQUEST['delete'] ) && $_REQUEST['delete'] > 0) {
		    $where = " where fin_rec_id = ".$_REQUEST['delete']."";
		    $deleteUser = $ketObj->runquery( "DELETE", "", "nar_fin_rec",array(),$where);
			$qstring ="";
			if (isset($_REQUEST['searchbyorder']) && $_REQUEST['searchbyorder']!="") {
				$qstring .= '&searchbyorder='.$_REQUEST['searchbyorder'].'';
			}
		    ...
		    header("Location: index.php?view=finance&file=received_tmpl".$qstring."");
		} 
}elseif (isset( $_REQUEST['chk'] ) && $_REQUEST['chk'] == 'rentrecdelete') {
  if (isset( $_REQUEST['delete'] ) && $_REQUEST['delete'] > 0) {
		    $where = " where rec_id = ".$_REQUEST['delete']."";
		    $deleteUser = $ketObj->runquery( "DELETE", "", "nar_recrent",array(),$where);
			$qstring ="";
			if (isset($_REQUEST['searchbyorder']) && $_REQUEST['searchbyorder']!="") {
				$qstring .= '&searchbyorder='.$_REQUEST['searchbyorder'].'';
			}
		    ...
		    header("Location: index.php?view=rent&file=rent_rec".$qstring."");
		} 
}